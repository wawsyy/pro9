...
export class FhevmDecryptionSignature {
  ...
  static async new(
    ...
    try {
      ...
      // Re-throw the error so the caller can handle it (e.g., user rejection)
      throw error;
    }
  }
  ...
  static async loadOrSign(
    ...
    if (cached) {
      console.log("Using cached FHE decryption signature (no wallet popup needed).");
      return cached;
    }

    console.log("No cached signature found. Requesting new signature from wallet...");
    ...
    try {
      ...
      // Re-throw to allow caller to handle (e.g., user rejection)
      throw error;
    }
  }
}
